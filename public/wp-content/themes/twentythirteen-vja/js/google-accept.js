...
function checkFirstVisit() {
   var consentCookie =  getCookie('cookie_notice_accepted'); 
...
*/
 ...
 function consent() {
    document.cookie = 'cookie_notice_accepted=true;'+ getCookieExpireDate() +' ; path=/';
    callGoogleAnalytics();
 }
...
if (!consentCookie) {//L'utilisateur n'a pas encore de cookie, on affiche la banniére et si il clique sur un autre élément que la banniére, on enregistre le consentement
	if ( notToTrack() ) { //L'utilisateur a activé DoNotTrack. Do not ask for consent and just opt him out
		...
		alert("Vous avez activé l'option \"DO NOT TRACK DNT\", nous respectons ce choix");
	} else {
...
//			document.onclick = consent;
	...
	if (document.cookie.indexOf('cookie_notice_accepted=false') > -1) // Aucun cookie n'est accepté
		window[disableStr] = true;
	else {
        if (document.cookie.indexOf(disableStr + '=true') > -1) 
         // les cookies sont acceptés mais google est explicitement refusé
           window[disableStr] = true;      
        else {   // les cookies sont acceptés et google n'est pas refusé 
            window[disableStr] = false;
            callGoogleAnalytics(); 
        }
